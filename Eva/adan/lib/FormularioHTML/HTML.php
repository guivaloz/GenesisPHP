<?php
/**
 * GenesisPHP - FormularioHTML HTML
 *
...
class HTML extends \Base\Plantilla {
    ...
    protected function elaborar_almacenar_imagen() {
        if (is_array($this->imagen)) {
            $a   = array();
            $a[] = "                    // Almacenar imagen";
            $a[] = "                    \$imagen = new \Base\ImagenHTML(parent::\$imagen_almacen_ruta, parent::\$imagen_tamanos);";
...
{$this->elaborar_almacenar_imagen()}
            ...
            } catch (\\Base\\BaseDatosExceptionSQLError \$e) {
                // Fallo el comando sql
                \$mensaje       = new \\Base\\MensajeHTML('Error SQL: Es posible que se haya ocasionado por la validación de la base de datos.<br />'.\$e->getMessage());
                \$mensaje->tipo = 'error';
        ...
        // Debe ser modificacion
            ...
            if (!\$this->sesion->puede_modificar('SED_CLAVE')) {
                \$mensaje = new \\Base\\MensajeHTML('Aviso: No tiene permiso para modificar SED_MENSAJE_SINGULAR.');
                return \$mensaje->html('Error');
            }
            // Consultamos
            try {
