<?php
/**
 * GenesisPHP - FormularioHTML RecibirFormulario
 *
...
class RecibirFormulario extends \Base\Plantilla {
    ...
    protected function elaborar_recibir_formulario_campos($columna, $datos) {
        ...
        switch ($datos['tipo']) {
            case 'caracter':
            case 'relacion':
                $campo = "\$this->$columna = \$this->post_select(\$_POST['{$columna}']);";
                break;
            case 'clave':
            case 'cuip':
            case 'curp':
            case 'mayusculas':
            case 'rfc':
                $campo = "\$this->$columna = \$this->post_texto_mayusculas_sin_acentos(\$_POST['{$columna}']);";
                break;
            case 'email':
                $campo = "\$this->$columna = \$this->post_texto_minusculas(\$_POST['{$columna}']);";
                break;
            case 'contraseña':
            case 'dinero':
            case 'entero':
            ...
            case 'telefono':
            case 'variable':
                $campo = "\$this->$columna = \$this->post_texto(\$_POST['{$columna}']);";
                break;
            case 'geopunto':
                $campo = "\$this->{$columna}_longitud = \$this->post_texto(\$_POST['{$columna}_longitud']);\n        \$this->{$columna}_latitud = \$this->post_texto(\$_POST['{$columna}_latitud']);";
                break;
            default:
                die("Error en FormularioHTML: Tipo {$datos['tipo']} no programado al recibir formulario.");
        }
        // Entregar campo
        return "        $campo";
