<?php
/**
 * GenesisPHP - BusquedaWeb Consultar
 *
...
class Consultar extends \Base\Plantilla {
    ...
    protected function elaborar_consultar_filtros_campo($tabla, $columna, $filtro, $datos, $relacion=null, $relacion_columna=null) {
        ...
        switch ($datos['tipo']) {
            ...
            case 'relacion':
                ...
                break;
            default:
                die("Error en BusquedaWeb, Consultar, elaborar_consultar_filtros_campo: Tipo {$datos['tipo']} no programado al elaborar consultar en $columna.");
        }
        // Entregar
        if ($a === false) {
    ...
    protected function elaborar_consultar_filtros_relacion($tabla, $columna, $datos) {
        ...
        if (is_array($this->relaciones[$columna])) {
            ...
            die("Error en BusquedaWeb, Consultar, elaborar_consultar_filtros_relacion: Falta obtener datos de Serpiente para la relación $columna.");
        }
        //
                ...
                if (is_array($vip_datos)) {
                    if ($vip_datos['tipo'] == 'relacion') {
                        if (is_array($this->relaciones[$vip])) {
                            ...
                            die("Error en BusquedaWeb, Consultar, elaborar_consultar_filtros_relacion: No está definido el VIP en Serpiente para $vip.");
                        }
                    } else {
    ...
    protected function elaborar_consultar_filtros() {
        ...
        if (count($a) > 0) {
            ...
            die('Error en BusquedaWeb, Consultar, elaborar_consultar_filtros: No hay columnas con filtro para crear el método recibir formulario.');
        }
    ...
    protected function elaborar_consultar_resultados_declaracion($instancia, $columna, $datos=false) {
        ...
        // Si hay filtro
            ...
            die("Error en BusquedaWeb, Consultar, elaborar_consultar_resultados_declaracion: No hay valor en filtro para $columna.");
        }
        // Entregar
        return implode("\n", $a);
    ...
    protected function elaborar_consultar_resultados_relacion($instancia, $columna, $datos) {
        ...
        if (is_array($this->relaciones[$columna])) {
            ...
            die("Error en BusquedaWeb, Consultar, elaborar_consultar_resultados_relacion: Falta obtener datos de Serpiente para la relación $columna.");
        }
        // Inicia
                ...
                if (is_array($vip_datos)) {
                    if ($vip_datos['tipo'] == 'relacion') {
                        if (is_array($this->relaciones[$vip])) {
                            ...
                            die("Error en BusquedaWeb, Consultar, elaborar_consultar_resultados_relacion: No está definido el VIP en Serpiente para $vip.");
                        }
                    } else {
    ...
    protected function elaborar_consultar_resultados() {
        ...
        if ($this->adan->si_hay_que_crear('tren')) {
            ...
            $instancia = '$tren';
            $a[] = '            // Entregar tren';
            $a[] = "            $instancia = new TrenWeb(\$this->sesion);";
        } elseif ($this->adan->si_hay_que_crear('listado')) {
            // Entregar un listado cuando el resultado sea mas de uno va
            $instancia = '$listado';
            $a[] = '            // Entregar listado';
            $a[] = "            $instancia = new ListadoWeb(\$this->sesion);";
        } else {
            die('Error en BusquedaWeb, Consultar, elaborar_consultar_resultados: Sin listado y sin tren no se puede mostrar el resultado de la búsqueda.');
        }
        // Para cada columna
        foreach ($this->tabla as $columna => $datos) {
            if (($datos['etiqueta'] == '') || ($datos['filtro'] == 0)) {
     ...
     * Consultar
     *
     * @return mixed Objeto con el ListadoWeb, TrenWeb o DetalleWeb, falso si no se encontró nada
     */
