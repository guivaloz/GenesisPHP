<?php
/**
 * GenesisPHP - BusquedaWeb Propiedades
 *
...
class Propiedades extends \Base\Plantilla {
    ...
    protected function elaborar_propiedades_declaracion($columna, $datos=false) {
        ...
        // Si hay filtro
            ...
            die("Error en BusquedaWeb, Propiedades, elaborar_propiedades_declaracion: No hay valor en filtro para $columna.");
        }
        // Entregar
        return implode("\n", $a);
        ...
        foreach ($this->tabla as $columna => $datos) {
            if (($datos['etiqueta'] == '') || ($datos['filtro'] == 0)) {
                ...
                if (is_array($this->relaciones[$columna])) {
                    ...
                    die("Error en BusquedaWeb, Propiedades, elaborar_propiedades: Falta obtener datos de Serpiente para la relación $columna.");
                }
                        ...
                        if (is_array($vip_datos)) {
                            if ($vip_datos['tipo'] == 'relacion') {
                                if (is_array($this->relaciones[$vip])) {
                                    ...
                                    die("Error en BusquedaWeb, Propiedades, elaborar_propiedades: No está definido el VIP en Serpiente para $vip.");
                                }
                            } else {
