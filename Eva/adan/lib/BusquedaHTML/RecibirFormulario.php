<?php
/**
 * GenesisPHP - BusquedaHTML RecibirFormulario
 *
...
class RecibirFormulario extends \Base\Plantilla {
    ...
    protected function elaborar_recibir_formulario_campo($columna, $datos) {
        ...
        switch ($datos['tipo']) {
            ...
            case 'relacion':
                ...
                break;
            default:
                die("Error en BusquedaHTML, RecibirFormulario, elaborar_recibir_formulario_campo: Tipo {$datos['tipo']} no programado al recibir formulario en $columna.");
        }
        // Entregar
        if ($a === false) {
    ...
    protected function elaborar_recibir_formulario_relacion($columna, $datos) {
        ...
        if (is_array($this->relaciones[$columna])) {
            ...
            die("Error en BusquedaHTML, RecibirFormulario, elaborar_recibir_formulario_relacion: Falta obtener datos de Serpiente para la relación $columna.");
        }
        //
    ...
    public function php() {
        ...
        if (count($a) > 0) {
            ...
            die('Error en BusquedaHTML, RecibirFormulario, php: No hay columnas con filtro para crear el método recibir formulario.');
        }
        // Entregar
        return <<<FINAL
    /**
