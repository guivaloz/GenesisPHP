<?php
/**
 * GenesisPHP - BusquedaHTML Validar
 *
...
class Validar extends \Base\Plantilla {
    ...
    protected function elaborar_validar_campo($columna, $datos, $relacion=null, $relacion_columna=null) {
        ...
        switch ($datos['tipo']) {
            ...
            case 'relacion':
                ...
                break;
            default:
                die("Error en BusquedaHTML, Validar, elaborar_validar_campo: No hay función para validar el tipo {$datos['tipo']} en $columna.");
        }
        ...
        if (is_string($funcion) && ($funcion != '')) {
            ...
            $a[] = "            throw new \\Base\\BusquedaHTMLExceptionValidacion('Aviso: {$datos['etiqueta']} incorrecto(a).');";
            $a[] = "        }";
        } elseif (is_array($funcion)) {
            // Es un rango, desde y hasta
            foreach ($funcion as $c => $f) {
                $a[] = "        if ((\$this->{$columna}_{$c} != '') && $f) {";
                $a[] = "            throw new \\Base\\BusquedaHTMLExceptionValidacion('Aviso: {$datos['etiqueta']} $c incorrecto(a).');";
                $a[] = "        }";
            }
        } else {
    ...
    protected function elaborar_validar_relacion($columna, $datos) {
        ...
        if (is_array($this->relaciones[$columna])) {
            ...
            die("Error en BusquedaHTML, Validar, elaborar_validar_relacion: Falta obtener datos de Serpiente para la relación $columna.");
        }
        // Inicia
                ...
                if (is_array($vip_datos)) {
                    if ($vip_datos['tipo'] == 'relacion') {
                        if (is_array($this->relaciones[$vip])) {
                            ...
                            die("Error en BusquedaHTML, Validar, elaborar_validar_relacion: No está definido el VIP en Serpiente para $vip.");
                        }
                    } else {
    ...
    protected function elaborar_validar_estatus() {
        ...
        if (($this->tabla['estatus'] != '') && ($this->tabla['estatus']['filtro'] > 0)) {
            ...
            $a[] = "                throw new \\Base\\BusquedaHTMLExceptionValidacion('Aviso: Estatus incorrecto.');";
            $a[] = "            }";
            $a[] = "        }";
        }
        // Entregar
    ...
    public function php() {
        ...
        if (count($a) > 0) {
            ...
            die('Error en BusquedaHTML, Validar, php: No hay columnas para crear el método validar.');
        }
        // Entregar
        return <<<FINAL
    /**
