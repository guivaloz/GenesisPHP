<?php
/**
 * GenesisPHP - ListadoHTML Constructor
 *
...
class Constructor extends \Base\Plantilla {
    ...
    protected function elaborar_constructor_recibir_filtros_declaracion($columna, $datos=false) {
        ...
        if ($datos['filtro'] > 1) {
            ...
            die("Error en ListadoHTML, Constructor, elaborar_constructor_recibir_filtros_declaracion: No hay valor en filtro para $columna.");
        }
        // Entregar
        return implode("\n", $a);
        ...
        foreach ($this->tabla as $columna => $datos) {
            if (($datos['etiqueta'] == '') || ($datos['filtro'] == 0)) {
                ...
                if (is_array($this->relaciones[$columna])) {
                    ...
                    die("Error en ListadoHTML, Constructor, elaborar_constructor_recibir_filtros: Falta obtener datos de Serpiente para la relación $columna.");
                }
                        ...
                        if (is_array($vip_datos)) {
                            if ($vip_datos['tipo'] == 'relacion') {
                                if (is_array($this->relaciones[$vip])) {
                                    ...
                                    die("Error en ListadoHTML, Constructor, elaborar_constructor_recibir_filtros: No está definido el VIP en Serpiente para $vip.");
                                }
                            } else {
            ...
            if ($datos['listado'] > 0) {
                ...
                if ($datos['tipo'] == 'relacion') {
                    ...
                    if (is_array($this->relaciones[$columna])) {
                        ...
                        die("Error en ListadoHTML, Constructor, elaborar_constructor_estructura: Falta obtener datos de Serpiente para la relación $columna.");
                    }
                    // Si vip es texto
                    if (is_string($relacion['vip']) && ($relacion['vip'] != '')) {
                        // Solo un vip
                            ...
                            if (is_array($vip_datos)) {
                                if ($vip_datos['tipo'] == 'relacion') {
                                    ...
                                    if (is_array($this->relaciones[$vip])) {
                                        ...
                                        die("Error en ListadoHTML, Constructor, elaborar_constructor_estructura: No está definido el VIP en Serpiente para $vip.");
                                    }
                        ...
                        die("Error en ListadoHTML, Constructor, elaborar_constructor_estructura: Falta el 'vip' en Serpiente para la relación $columna.");
                    }
    ...
    protected function elaborar_constructor_viene_listado() {
        ...
        if (count($b) == 0) {
            die('Error en ListadoHTML: Es necesario que haya por lo menos una columna como filtro.');
        }
