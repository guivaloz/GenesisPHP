...
class HTML extends \Base\Plantilla {
    ...
    protected function elaborar_html_accion_imprimir() {
        ...
        if ($this->padre === false) {
            die('Error en Página: Este módulo con contenido impresiones no tiene padre. Debe tener uno por lo menos.');
        }
...
if ((\$_GET['{$padre['instancia_singular']}'] != '') && (\$_GET['caracteresazar'] != '')) {
                // Impresion
                \$impresion = new ImpresionHTML(\$this->sesion);
...
FIN;
    ...
    protected function elaborar_html_accion_modificar() {
        ...
        // Entregar
        return <<<FIN
if ((\$_GET['{$this->primary_key}'] != '') && (\$_GET['accion'] == DetalleHTML::\$accion_modificar)) {
                // Modificar
...
FIN;
    ...
    protected function elaborar_html_accion_eliminar() {
        ...
        // Entregar
        return <<<FIN
if ((\$_GET['{$this->primary_key}'] != '') && (\$_GET['accion'] == DetalleHTML::\$accion_eliminar)) {
                // Eliminar
...
FIN;
    ...
    protected function elaborar_html_accion_recuperar() {
        ...
        // Entregar
        return <<<FIN
if ((\$_GET['{$this->primary_key}'] != '') && (\$_GET['accion'] == DetalleHTML::\$accion_recuperar)) {
                // Recuperar
...
FIN;
    ...
    protected function elaborar_html_accion_detalle() {
        ...
        if (is_array($this->hijos) && (count($this->hijos) > 0)) {
            ...
            $d[] = "                // Detalle e hijos";
            $d[] = "                \$detalle = new DetalleHTML(\$this->sesion);";
    ...
    protected function elaborar_html_accion_formulario_recibir() {
        ...
        // Juntaremos el codigo en este arreglo
        $a = array();
        $a[] = "if (\$_POST['formulario'] == FormularioHTML::\$form_name) {";
        $a[] = "                // Viene el formulario";
        $a[] = "                \$formulario = new FormularioHTML(\$this->sesion);";
        // Si tiene hijos
        if (is_array($this->hijos) && (count($this->hijos) > 0)) {
            // Tiene hijos
...
FIN;
    ...
    protected function elaborar_html_accion_listados() {
        ...
        if (is_array($this->estatus)) {
            ...
            if (($this->estatus['enuso'] = 'A') && ($this->estatus['eliminado'] == 'B')) {
                $l[] = "            // Listados";
                $l[] = "            \$listado = new ListadoHTML(\$this->sesion);";
    ...
    protected function elaborar_html_accion_trenes() {
        ...
        if (is_array($this->estatus)) {
            ...
            if (($this->estatus['enuso'] = 'A') && ($this->estatus['eliminado'] == 'B')) {
                $t[] = "            // Trenes";
                $t[] = "            \$tren = new TrenHTML(\$this->sesion);";
