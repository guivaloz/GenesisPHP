<?php
/**
 * GenesisPHP - PaginaHTML CollapsePadreEHijos
 *
...
class CollapsePadreEHijos extends \Base\Plantilla {
    ...
    protected function elaborar_html_hijos() {
        ...
        foreach ($this->hijos as $hijo) {
            ...
            if ($hijo['listados'] == 'trenes') {
                $h[] = "                {$instancia} = new \\{$clase}\\TrenHTML(\$this->sesion);";
            } else {
                $h[] = "                {$instancia} = new \\{$clase}\\ListadoHTML(\$this->sesion);";
            }
     ...
     *
     * @param  mixed Instancia de DetalleHTML, BusquedaHTML con el id cargado
    ...
    protected function crear_collapse_padre_hijos(\$in_instancia) {
        // Si es una busqueda
        if (\$in_instancia instanceof \\Base\\BusquedaHTML) {
            // Ejecutar metodo html de la busqueda
            \$html = \$in_instancia->html();
            if (\$in_instancia->hay_resultados && (\$in_instancia->resultado instanceof DetalleHTML)) {
                // Tiene un DetalleHTML que procesaremos
                \$detalle = \$in_instancia->resultado;
            } else {
                // Es un listado o formulario, se regresa
                return \$in_instancia;
            }
        } elseif (\$in_instancia instanceof DetalleHTML) {
            // Es un DetalleHTML
