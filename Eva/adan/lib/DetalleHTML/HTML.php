...
class HTML extends \Base\Plantilla {
    ...
    protected function elaborar_html_detalle_propiedades_relacionadas($columna, $datos) {
        ...
        if (is_array($this->relaciones[$columna])) {
            ...
            die("Error en DetalleHTML: Falta obtener datos de Serpiente para la relación $columna.");
        }
        // Si vip es texto
                ...
                if (is_array($vip_datos)) {
                    ...
                    if ($vip_datos['tipo'] == 'relacion') {
                        ...
                        if (is_array($this->relaciones[$vip])) {
                            ...
                            die("Error en DetalleHTML: No está definido el VIP en Serpiente para $vip.");
                        }
    ...
    protected function elaborar_html_detalle_propiedades() {
        ...
        if (count($a) > 0) {
            ...
            die('Error en DetalleHTML: No hay columnas con etiqueta en la tabla para mostrar.');
        }
...
FINAL;
    ...
    protected function elaborar_html_detalle_seccion_imagen() {
        ...
        if ($caracteres == '') {
            die('Error en DetalleHTML: Al arreglo imagen le falta el valor para "caracteres".');
        }
        if ($this->imagen['tamaños']['middle'] == '') {
            die('Error en DetalleHTML: Al arreglo imagen le falta el tamaño "middle".');
        }
        // ENTREGAR
        return <<<FINAL
        // Agregar imagen al detalle
        \$imagen = new \\Base\\ImagenHTML(parent::\$imagen_almacen_ruta, parent::\$imagen_tamanos);
...
FINAL;
    ...
    protected function elaborar_html_detalle_seccion_mapa() {
        ...
        foreach ($this->mapa as $columna => $datos) {
            ...
            if ($this->tabla[$columna]['tipo'] == 'geopunto') {
                ...
                $a[] = "        \$mapa = new \\Base\\MapaHTML('detalle');";
    ...
    protected function elaborar_html_detalle() {
        ...
        $a[] = "        // Crear detalle";
        $a[] = "        \$detalle = new \\Base\\DetalleHTML();";
