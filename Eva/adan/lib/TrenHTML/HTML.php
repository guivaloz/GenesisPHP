...
class HTML extends \Base\Plantilla {
    ...
    protected function elaborar_html_alimentar_vagones() {
        ...
        if (is_array($this->imagen)) {
            ...
            if ($caracteres == '') {
                die('Error en TrenHTML, HTML, elaborar_html_alimentar_vagones: Al arreglo imagen le falta el valor para "caracteres".');
            }
            if ($this->imagen['tamaños']['small'] == '') {
                die('Error en TrenHTML, HTML, elaborar_html_alimentar_vagones: Al arreglo imagen le falta el tamaño "small".');
            }
        } else {
            die('Error en TrenHTML, HTML, elaborar_html_alimentar_vagones: Propiedad imagen no está definida.');
        }
        // Alimentar vagones
        $a[] = "        // Alimentar los vagones con instancias de cada imagen";
        $a[] = "        \$imagen = new \\Base\\ImagenHTML(Registro::\$imagen_almacen_ruta, Registro::\$imagen_tamanos);";
...
{$this->elaborar_html_alimentar_vagones()}
        ...
        \$this->tren_controlado->limit              = \$this->limit; // Pasamos el limit, porque desde la pagina se puede pedir sin botones
