...
class Cadenero extends Registro {
    ...
    public function consultar($in_clave='') {
        ...
        try {
            ...
            throw new BaseDatosExceptionSQLError($this->sesion, 'Error en cadenero: Al consultar en cadenero. ', $e->getMessage());
        }
        // Si no se encuentra, provoca excepcion
        if ($consulta->cantidad_registros() < 1) {
            throw new CadeneroExceptionNoEncontrado($this->sesion, $this->form_name, 'No se encontró.');
        }
        // Definir propiedades
    ...
    public function validar_recepcion($in_form_name, $in_clave) {
        ...
        if ($in_form_name !== $this->form_name) {
            throw new CadeneroExceptionNoEncontrado($this->sesion, $in_form_name, 'No coincide el formulario.');
        }
        ...
        if ($this->usuario != $this->sesion->usuario) {
            throw new CadeneroExceptionNoEncontrado($this->sesion, $this->form_name, 'No coincide el usuario.');
        }
        // Si ya fue recibida, provoca excepcion
        if ($this->recibido == true) {
            throw new CadeneroExceptionYaRecibido($this->sesion, $this->form_name);
        }
        ...
        try {
            ...
            throw new BaseDatosExceptionSQLError($this->sesion, 'Error en cadenero: Al actualizar registro para cambiar recibido a verdadero. ', $e->getMessage());
        }
        // Entregar verdadero
        return true;
    } // validar_recepcion
